...
async def store_message(token: str, msg: dict, chat_id: str):
    ...
    async with chats_lock:
        if key not in chats_messages:
            info = await fetch_chat_info(token, chat_id)
            chats_messages[key] = {"title": info["title"], "photo": info["photo"], "type": info["type"], "messages": []}
        chat = chats_messages[key]
...
async def delete_bot(data: dict):
    ...
    to_delete = []
    async with chats_lock:
        for key in list(chats_messages.keys()):
            if key.startswith(f"{token}:"):
                to_delete.append(key)
        for key in to_delete:
            chats_messages.pop(key, None)
    return {"status": "ok"}
...
async def get_chats(token: str):
    ...
    prefix = f"{token}:"
    async with chats_lock:
        for key, chat in chats_messages.items():
...
async def get_messages(chat_id: str, token: str):
    key = f"{token}:{chat_id}"
    async with chats_lock:
        chat = chats_messages.get(key)
        if not chat:
